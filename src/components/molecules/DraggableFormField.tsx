...
export default function DraggableFormField({
  ...
  return (
    ...
    >
      <div className="flex absolute -top-2 -right-2 flex-row items-center justify-between">
        <button
          onClick={() => onRemove(field.id)}
          className="w-6 h-6 bg-red-600 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-700 flex items-center justify-center text-xs z-10"
        >
          ×
        </button>
        {/* Drag handle */}
        <div className="opacity-0 group-hover:opacity-100 transition-opacity cursor-move z-10">
